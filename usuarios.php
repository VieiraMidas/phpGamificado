...
<body>
    ...
    <div class="col-10">
        ...
        <button class="btn btn-dark" data-toggle="modal" data-target="#newmodal">
            ...
            <table class="table">
                ...
                <?php foreach ($lista as $users): ?>
                    <tr>
                        ...
                        <td><?=$users->Email?></td>
                        <td>
                            <button class="btn btn-dark alterar-email" data-id="<?=$users->UserID?>"><i class="fas fa-envelope-square" data-toggle="modal" data-target="#modalemail"></i></button>
                            <button class="btn btn-warning alterar-senha" data-id="<?=$users->UserID?>"><i class="fas fa-key" data-toggle="modal" data-target="#modalsenha"></i></button>
            ...
            <!--modal troca email-->
            <div class="modal fade" id="modalemail" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLongTitle">Alterar Email e Nome</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form action="usercontrol.php?acao=trocaEmail" method="POST">
                                <div class="input-group mb-3">

                                </div>
                                <input type="hidden" name="id" id="campo2-id">
                                <div class="input-group mb-3">
                                    <input type="text" name="nome" class="form-control" placeholder="Novo Nome" aria-label="Username" aria-describedby="basic-addon1">
                                </div>
                                <div class="input-group mb-3">
                                    <input type="text" name="email" class="form-control" placeholder="novo E-mail" aria-label="Username" aria-describedby="basic-addon1">
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Sair</button>
        ...
        botao.addEventListener("click", function(){
            //window.alert(botao.getAttribute("data-id"));
            var campo = document.querySelector("#campo2-id");
            campo.value = botao.getAttribute("data-id");
        });
    </script>
    </html>